...
export const api = {
  ...
  async sendEmailNotification(to: string, subject: string, body: string): Promise<void> {
    ...
    await Promise.all([to, ...adminEmails].map(email => sendTo(email)));
  }
};
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ requests, users, onUpdateStatus, onDelete, onApproveReset }) => {
  ...
  const handleApproval = async (req: AidRequest, committee: 'SCIENTIFIC' | 'ADMIN') => {
    ...
    // Se ambos aprovaram, dispara e-mail
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-2 pt-4 border-t items-center">
                {selectedRequest.status === RequestStatus.PENDING_APPROVAL && (
                  <>
                    <button onClick={() => handleApproval(selectedRequest, 'SCIENTIFIC')} className={`flex-1 py-2 rounded font-bold ${selectedRequest.scientificApproved ? 'bg-green-100 text-green-700' : 'bg-green-600 text-white'}`}>
                      {selectedRequest.scientificApproved ? 'Científico OK' : 'Aprovar Científico'}
                    </button>
                    <button onClick={() => handleApproval(selectedRequest, 'ADMIN')} className={`flex-1 py-2 rounded font-bold ${selectedRequest.adminApproved ? 'bg-green-100 text-green-700' : 'bg-green-600 text-white'}`}>
                       {selectedRequest.adminApproved ? 'Admin OK' : 'Aprovar Admin'}
                    </button>
                ...
                {(selectedRequest.status === RequestStatus.ACCOUNTABILITY_REVIEW) && (
                   <button onClick={() => { onUpdateStatus(selectedRequest.id, RequestStatus.WAITING_REIMBURSEMENT); setSelectedRequest(null); }} className="flex-1 bg-citi-600 text-white py-2 rounded font-bold">Aprovar Contas & Finalizar</button>
                )}
                 {(selectedRequest.status === RequestStatus.WAITING_REIMBURSEMENT) && (
                   <button onClick={() => { onUpdateStatus(selectedRequest.id, RequestStatus.COMPLETED); setSelectedRequest(null); }} className="flex-1 bg-blue-600 text-white py-2 rounded font-bold">Confirmar Pagamento</button>
                )}
                
                <button onClick={() => setDeleteConfirmId(selectedRequest.id)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded ml-2" title="Excluir Solicitação"><Trash2 size={20} /></button>
              </div>
            </div>
          </div>
        </div>
...
const App: React.FC = () => {
  ...
  const handleNewRequest = (newReqData: any) => {
    ...
    api.saveRequest(newReq).then(async () => { 
       await api.sendEmailNotification(newReq.employeeInputName, "Solicitação Enviada - CITI", `Sua solicitação para ${newReq.eventName} foi enviada com sucesso.`);
       refreshData(); 
  ...
  const handleAccountabilitySubmit = (reqId: string, docs: SimpleFile[]) => {
    ...
    if (req) {
        api.saveRequest({ ...req, status: RequestStatus.ACCOUNTABILITY_REVIEW, accountabilityDocuments: docs }).then(async () => { 
            await api.sendEmailNotification(req.employeeInputName, "Prestação de Contas Enviada", `Documentos enviados para o evento ${req.eventName}.`);
            refreshData(); 
  ...
  const handleAdminStatusUpdate = async (id: string, newStatus: RequestStatus) => {
    ...
    if (req) {
        ...
        let msg = "";
        if(newStatus === RequestStatus.WAITING_REIMBURSEMENT) msg = "Sua prestação de contas foi aprovada. O reembolso entrou na fila.";
        if(newStatus === RequestStatus.COMPLETED) msg = "O reembolso foi realizado e o processo finalizado.";
        
        if(msg) await api.sendEmailNotification(req.employeeInputName, "Atualização - CITI", msg);
        refreshData();
    }
  };
